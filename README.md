...
|**Ім'я поля**|**Тип**|**Опис**|
|:---|:---|:---|
|point_number    |Long   |посилання на ID клієнта [point.point_number](#point)|
...
## <a id="list">LIST
Таблиця товарних записів документа. Посилається на [Nakl.nakl_number\](#nakl)
|**Ім'я поля**|**Тип**|**Опис**|
|:---|:---|:---|
...
|reestr_number|Long  |ID реєстрового запису з таблиці [reestr](#reestr). Може набувати значення 0|
...
## <a id="nakl">NAKL
Таблиця документів. На неї посилається таблиця товарних записів [List.nakl_number](#list)
Таблиці [Nakl](#nakl) і [List](#list) використовуються для зберігання документів "Замовлення постачальнику", з яких на касі створюються прибуткові накладні. Касир має доступ до переліку замовлень постачальнику. При імпорті замовлення відразу ж буде створена прибуткова накладна з повним переліком товарів з цього замовлення.
|**Ім'я поля**|**Тип**|**Опис**|
|:---|:---|:---|
...
|**Ім'я поля**|**Тип**|**Опис**|
|:---|:---|:---|
|SalePricesWithExciseTax|String|Містить список цін продажу з включеним у себе акцизом, розділених точкою з комою. Приклад: 99,75; 105; 115,50; 120,75; 99,75; 84; 94,50.<br>Ці ціни використовуються для продажу акцизних товарів із рекомендованою ціною (нанесеною на упаковку).<br>Обов'язково до таблиці TCU_PAR необхідно прописати розмір акцизної ставки цього товара. Повинен бути рядок із групою, що відповідає податковій ставці товара. Наприклад, товар має податкову групу 1, тому **TCU_PAR.param_name = FiscalGroup1TaxValue, TCU_PAR.param_type = 3, TCU_PAR.double_value = 0,05 (відповідає 5%).**<br>Група податкової ставки товара вказується у [tovar.fiscal_group_number](#tovar)<br>З цін продажу у вищезгаданому списку будуть відніматися ці 5% і ціни без акцизу (ціна без акцизу відповідає ціні, зазначеній на упаковці товара) будуть показані касиру для вибору.|
...
Якщо використовується фіскальний реєстратор, Shopdesk на робочому місці касира звіряє значення податкових ставок, прописаних в самому пристрої зі значеннями в обліковій системі. Це простий контроль для виключення помилок при визначенні податкових ставок для товарів (див. [Tovar.fiscal_group_number](#tovar)), який, однак, не перешкоджає реєстрації продажів навіть за наявності помилок або невказаної податкової ставки. Проте настійно рекомендується для відповідної податкової групи вказати податкову ставку в цій таблиці, щоб на робочому місці касира не з'являлося попередження про розбіжність податкових ставок.
...
|**Ім'я поля**|**Тип**|**Опис**|
|:---|:---|:---|
|franch_top_id                               |Long               |Для франчайзі товарів у полі franch_top_id має бути посилання на ID товара у базі CRM. Для нефраншизних товарів = 0|
|discount_limitation                         |Double             |Максимально можлива знижка на товар. Може набувати значень від 0 до 1. «1» відповідає 100% знижці.|
|fiscal_group_number                         |Integer            |Податкова група товара. Визначає, за якою податковою групою має реєструватися товар у фіскальному реєстраторі (ФР). Відповідає індексу групи у ФР. <br>Наприклад, у ФР існують такі групи: А, Б, В, Г, М. Тоді відповідність груп та значень має вигляд:<br><table><tbody><tr><th>Група</th><th>fiscal_group_number</th><th>Група в картці товара в ТЦУ</th></tr><tr><td>А (20%)</td><td>0</td><td>(1) 20.00%</td></tr><tr><td>Б (0%)</td><td>1</td><td>(2) 20.00%</td></tr><tr><td>В (0%)</td><td>2</td><td>(3) 20.00%</td></tr><tr><td>Г (0%)</td><td>3</td><td>(4) 20.00%</td></tr><tr><td>М (0%+5%)</td><td>6</td><td>(7) 5.00%</td></tr></tbody></table><br>Залежно від типу реєстратора, індекс груп вище 4-ї (Група Г) слід визначати практично. Важливо використовувати реєстратори одного типу та з однаково встановленими значеннями податкових ставок на всіх торгових точках, які працюють із єдиним довідником товарів.|
|image1                                      |image(бінарні дані)|Зображення товара. Формат зображення – jpg, png. Розмір – не більше 300 точок за висотою чи шириною. Картинка буде вписана в елемент інтерфейсу при будь-якому співвідношенні сторін, однак для кращого результату використовуйте співвідношення сторін висота/ширина від 0,87 до 1. На зображенні товара слід уникати текстів, написів у нижній частині картинки, оскільки у інтерфейсі касового ПЗ там буде накладено назву товара.|
|tov_picture                                 |String             |Назва файлу зображення товара|
|cena_rozn_sprav                             |Double             |Довідкова ціна 1|
|cena_rozn2                                  |Double             |Довідкова ціна 2|
|cena_rozn3                                  |Double             |Довідкова ціна 3|
|cena_rozn4                                  |Double             |Довідкова ціна 4|
|cena_rozn5                                  |Double             |Довідкова ціна 5|
|cena_rozn6                                  |Double             |Довідкова ціна 6|
|cena_rozn7                                  |Double             |Довідкова ціна 7|
|cena_rozn8                                  |Double             |Довідкова ціна 8|
|cena_rozn9                                  |Double             |Довідкова ціна 9|
|cena_rozn10                                 |Double             |Довідкова ціна 10|
|cena_rozn11                                 |Double             |Довідкова ціна 11|
|cena_rozn12                                 |Double             |Довідкова ціна 12|
|cena_rozn13                                 |Double             |Довідкова ціна 13|
|cena_rozn14                                 |Double             |Довідкова ціна 14|
|cena_rozn15                                 |Double             |Довідкова ціна 15|
|IsExciseMarkBarcodeRequiredForSaleOnShopdesk|Boolean            |Вимагати на касі введення штрих-коду акцизної марки (назва поля не має переносів)|
...
|SaleStatus                                  |Integer            |Нумератор, що вказує на можливість продажу товара0 або 1 - товар відкритий для продажу2 - товар відкритий для продажу тільки в тих випадках, якщо його залишок після продажу >=0 (іншими словами, продажі в мінус заборонені, аналогічно тому випадку, коли касир не має права на відвантаження в мінус)4 - товар закритий для продажу|
|MinimalCustomerAge                          |Integer            |Мінімальний вік клієнта у роках, з якого йому дозволено продаж цього товара. Під час продажу цього товара з’являється форма, у якій треба зазначити вік клієнта.|
|ShelfLife                                   |Integer            |Термін придатності товара у днях. Використовується для друку терміну придатності вагового товара під час друку на етикетках|
